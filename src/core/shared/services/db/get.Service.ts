    ...
    try
    {
      ...
      if (sort && sort.by.length > 0) {
        sort.by.forEach(column => {
          queryBuilder = queryBuilder.addOrderBy(`entity.${column}`, sort.direction === Order.ASC ? 'ASC' : 'DESC');
        });
      }
      ...
      if (pagination) {
        pagedList = await PagedList.toPagedListAsync(queryBuilder, pagination.pageNumber, pagination.pageSize);
      } else {
