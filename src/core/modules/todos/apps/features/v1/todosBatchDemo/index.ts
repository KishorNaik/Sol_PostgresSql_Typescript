...
export interface ITodosBatchDemoService extends IServiceHandlerAsync<ITodosBatchDemoServiceParameters,ToDoEntity[]>{
}
...
export class TodosBatchDemoService implements ITodosBatchDemoService{
  ...
  public async handleAsync(params: ITodosBatchDemoServiceParameters): Promise<Result<ToDoEntity[], ResultError>> {
    try
    {
      if(!params)
